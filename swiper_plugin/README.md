...
1. `html` 파일 내에서 슬라이드삽입위치에 다음과 같이 태그작성
...
3. 프로젝트에 디자인에 따라 필요한 추가 태그들을 작성
    `div.swiper-slide` 안에 `a,h,p,table,form,div` 등 작성

4. CSS 선택자 준비 (개별 , 공통용 모두)
    * **주의사항**
    * `swiper-wrapper , swiper-slide`
        `display , width , height , position ` 적용하지않기
    * `swiper` 제일 바깥쪽 swiper 시작태그에만 크기 적용
...
## Swiper {option:value,} 종류와 뜻
    * loop:(true / false) : 슬라이드의 시작과 끝을 반복 이어주는?
    * direction:(horizontal / vertical ) : 슬라이드의 방향을 수직 , 수평으로 설정
    * mousewheel:(true/false) : 슬라이드에 마우스 가져다대고 휠을 쓰면 슬라이드가 움직임
    * effect:(fade / cube/ flip ) : 슬라이드가 넘어갈때의 애니메이션을 바꿔줌
    * autoplay:{ : 슬라이드를 자동으로 넘겨줌
        delay:1000 (1000=1s) : 자동으로 넘겨주는 시간 설정
        pauseOnMouseEnter:(true/false) : 마우스를 슬라이드에 가져다대면 일시정지
        disableOnInteraction:(true/false) : 마우스를 상호작용 했을때 슬라이드의 멈춤/움직임 설정
    }
...
### 기존 Swiper_slide와 겹치는 디자인을 하고싶을때
* 태그 위치 : `div.swiper` 두번째 자식 위치로 생성
### 기존 Swiper_slide 외부 바깥쪽으로 안겹치는 디자인을 하고 싶을 때
* 태그 위치 : `div.swiper` 다음 형제 위치로 생성
* `div.swiper`와 생성된 선택태그를 묶어주는 추가 태그 생성(디자인에 따라 다름)
### 교차 여부에 따라 태그 위치를 다르게 해야하는 이유
* `div,swiper`에 기본 CSS로 `overflow:hidden`이 있으므로 그 값을 `visible`로 바꾸면 선택요소 뿐 아닌 `swiper-slide` 들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.

## Swiper 안에 추가 Swiper 생성 시 안쪽 Swiper 마우스드래그 안될때 해결방법
...
1. `init:function(){}`
    ...
    * `on:{ init:function(){}, },`
2. `slideChangeTransitionStart:funtion(){}`
