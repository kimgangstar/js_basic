...
data2 = '가나다라';
data3 = data2 * 10; //null 값을 삭제할때 대입하는 키워드
typeResult = Boolean(data3);
console.log(`data1의 데이터타입은 : ${typeResult}`);
...
data3 = []; //빈 배열 new Array(); 내장함수가 있다로 구분
// 빈 문자열은 거짓
typeResult = Boolean(data3);
...
// if(조건식){조건식이 참일 때}else{조건식이 거짓일때 실행}
...
x = '5';
y = 5;
if( x==y ) console.log('x와y는 같다');
x = Number(x); //x를 숫자로 변경
if( x===y ) console.log('x와y는 데이터타입까지 같다.');

let w = 1920;
let h = 1080;
if( w!=h ) console.log('w와h는 다르다');
